...
def _audio_diarize(wav_file):
    ...
    """
    diarize_model = config.conf.select("audio.diarization_model", default="pyannote/speaker-diarization-3.1")
    device = config.conf.select("device", default="mps")
    num_speakers = config.conf.select("audio.num_speakers", default=2)
    min_speakers = config.conf.select("audio.min_speakers", default=2)
...
def _parse_diarization_result(diarization: str):
...
def _parse_transcript_to_diarization(transcript: str):
...
def _save_diarization_transcript(audio_segments, diarization_segments, output_file: Path):
    final_transcription = _align_transcription_with_speakers(audio_segments, diarization_segments)
    output_file.write_text("\n".join(final_transcription))
    logger.debug(f"Diarized transcription saved at {output_file}")


def _align_transcription_with_speakers(audio_segments, diarization_segments):
    """
    ...
    Input:
        audio_segments (str): Parsed raw audio transcription.
        diarization_segments (str): Parsed raw audio diarization.
    ...
    grouped_transcription = _group_transcription_by_speaker(aligned_transcription)
    final_transcription = []
    ...
    for segment in grouped_transcription:
        final_transcription.append(
            ...
            # Start-end segment is still included for easier check
        )

    return final_transcription


def _group_transcription_by_speaker(aligned_transcription):
